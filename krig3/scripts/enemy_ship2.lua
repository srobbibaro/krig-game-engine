...
function on_update(this, elapsedTime)
  ...
  if ( this.position[2] >= 15 ) then
    ...
    -- Angle a bit down.
    if ( relative_pos < 5 ) then this.velocity = {2.0, -5.0, 0.0} end
    -- Go straight down.
    if ( relative_pos < 0 ) then this.velocity = {10.0, -10.0, 0.0} end
  ...
  if ( this.position[2] < 15 ) then
    ...
    if this.position[2] < 10 then this.velocity = {18.0, -5.0, 0.0} end
    -- Full retreat.
    if relative_pos > 5 then this.velocity = {20.0, 0.0, 0.0} end
