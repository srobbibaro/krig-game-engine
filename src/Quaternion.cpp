...
void Quaternion::buildFromEuler( const Vector &v )
{
    ...
    float cosX = cos(v.x/2);
	float cosY = cos(v.y/2);
	float cosZ = cos(v.z/2);

    float sinX = sin(v.x/2);
	float sinY = sin(v.y/2);
	float sinZ = sin(v.z/2);
...
{
    // calculate trig identities ONCE
    float cosX = cos(angleX/2);
	float cosY = cos(angleY/2);
	float cosZ = cos(angleZ/2);

    float sinX = sin(angleX/2);
	float sinY = sin(angleY/2);
	float sinZ = sin(angleZ/2);
